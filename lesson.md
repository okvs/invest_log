...
## 2026-06-12
### 증거금률 ≠ 현금 부담률 — KB 자기융자는 전액 융자로 체결된다
- **문제**: 신용융자 잔액이 봇 6,593만 vs 실제 1.27억(−6,107만 과소), 예수금도 −1,961만 과소. 장부 재생은 1원까지 맞는데 실측과 어긋남.
- **원인**: 봇이 신용매수를 `현금 = total×증거금률, 융자 = total×(1−증거금률)`로 모델링("신용 45%" 버튼 = 현금 45%+신용 55%). 그러나 **KB 자기융자는 종목 증거금률(45% 등)과 무관하게 매수금 전액이 융자**로 체결되고 증거금은 담보로 묶일 뿐 현금이 차감되지 않는다(사용자 실측 발견). 매 신용매수마다 현금은 과소·융자는 과소 기록이 쌓였고, 중간의 예수금 강제보정이 현금 쪽 오차를 흡수해 더 오래 숨었다.
- **해결**: `margin:0`(전액 신용) 옵션 추가 — 기존 산식이 0을 정확히 처리(credit=100%, 현금차감 0). 라벨도 "신용 60%"(오독: 실제 의미는 현금60+신용40) → "현금60+신용40"으로 분해 표기. 누적분은 실측 기반 reconcile(신용 1.27억 비례 배분).
- **교훈**: (1) **브로커 체결의 자금 구성은 약관/상품이 결정한다** — "증거금률 = 현금 부담률"은 검증 안 된 가정이었고, 실계좌 대조 전까지 6,107만이 조용히 쌓였다. (2) 도메인 용어(증거금률)를 UI 라벨로 그대로 쓰면 사용자는 자기 브로커 화면의 숫자를 입력한다 — 회계적 의미(현금/신용 분해)를 라벨에 직접 적을 것. (3) 잔고류 실측 대조는 보유 수량만으론 부족 — **예수금·융자잔액까지 3종 세트로 대조**해야 자금 구성 오류가 드러난다.
### yfinance .KS 는 장 시작 후에도 전일 데이터를 준다 — 당일 등락률 비교에 쓰면 허위 신호
- **문제**: 현선물 괴리 알림이 "삼성전자 선물 333,000(+11.0%) vs 현물 299,000(−1.2%) = 괴리 +12.2%p"를 발송. 현물 299,000은 전일(6/11) 종가였고 −1.2%는 전전일(6/10) 종가 대비 — 실제 현물은 같은 시각 33만원대(+13%), 진짜 괴리는 +0.3%p. 사용자가 "현물 가격이 엄청 옛날 가격"이라고 신고.
- **원인**: 괴리 측정의 현물 쪽이 yfinance `fast_info.last_price`(.KS)인데, **Yahoo 의 KRX 데이터는 개장(09:00) 후에도 한동안(이날 09:17까지도) 전일 세션 데이터를 반환**한다. 이때 `last_price`=전일종가, `previous_close`=전전일종가로 하루씩 밀려 가격·등락률이 모두 오염된다. 선물 쪽은 KIS 실시간이라 신선 → "신선한 선물 vs 하루 묵은 현물" 비교가 구조적 허위 괴리를 만든다. 감시 시작도 08:00이라 개장 전(현물 미거래) 측정까지 하고 있었다.
- **해결**: (1) `kis_futures.fetch_kis_spot_quote`(FHKST01010100, 스로틀+레이트리밋 재시도+5분 캐시) 신설, `fetch_futures_quotes`의 기초자산 시세를 KIS 우선·yfinance 폴백으로 교체하고 `underlying_source` 태그 추가. (2) 괴리 판정에서 `underlying_source=='yfinance'` 제외(현물도 KIS일 때만 측정). (3) 감시 시작 08:00→09:05.
- **교훈**: (1) **이종 소스 간 "당일 변동률" 비교는 두 소스의 기준시점이 같을 때만 유효** — 한쪽이 실시간이면 다른 쪽도 실시간이어야 하고, 폴백 소스로 내려가면 비교 자체를 포기하는 게 맞다. (2) yfinance .KS 는 종가 시계열용으로는 충분하지만 **장중 실시간·당일 등락률 용도로는 신뢰 불가**(지연이 아니라 세션 단위 stale). (3) 시세 비교 메시지에는 소스를 함께 기록하면(이번에 `underlying_source` 추가) 다음 신고 때 로그만으로 원인이 즉시 갈린다.
