...
2. **스샷 파싱(비전)** — `Read` 로 `image_path` 의 이미지를 읽는다. 증권사 '잔고' 화면에서
   **종목별 융자/신용/대출 금액(원)** 을 추출한다. 화면 레이아웃이 증권사마다 다르니 유연하게:
   - "융자", "신용", "대출", "신용융자", "융자금" 등의 라벨/컬럼을 종목과 매칭.
...
3. **현재 기록 조회** — 한 번 실행:
   ...
   ```
   출력 JSON(`holdings`: name/quantity/avg_price/credit_loan)을 현재 상태로 삼는다.

4. **대조** — 스샷에서 읽은 값과 현재 기록을 맞춘다(종목명은 공백/대소문자 무시 매칭).
   - **융자(credit_loan)**: 스샷 값으로 갱신할 `{종목명: 융자원}` 맵을 만든다. 현재 보유에 없는
...
5. **융자 반영** — 4에서 만든 맵을 JSON 으로 넘겨 적용(+대시보드 재발행):
   ```bash
   cd /Users/seung/.openclaw/workspace/invest_log && .venv/bin/python scripts/balance_apply.py apply <req_id> '{"SK하이닉스": 52000000, "삼성전자": 39140000}'
   ```
   출력 JSON 의 `changes`(name/old/new 원), `unmatched`, `total_loan` 을 회신에 쓴다.
