...
- **해결/교훈**:
  1. **시각 취향은 말로 받지 말고 실물 3안을 렌더해 텔레그램으로 보내 고르게** 하니 한 번에 수렴(안 C 선택). ASCII 프리뷰보다 실제 PNG가 압도적으로 빠름.
  2. **마커가 봉을 가린다/연결선이 윗꼬리로 보인다** → 마커는 봉 밖(매수 아래·매도 위)으로 충분히 이격, 가격은 봉 위 노란 **점선** 가로줄로 분리 표기, 평단선은 회색으로 색 충돌 회피.
  3. **인터랙티브 hover + ≥임계 봉 강조 = plotly**가 정답. `make_subplots`(가격+거래량), 캔들 hover 는 까다로우니 **투명 scatter 오버레이 + hovertemplate(customdata)** 로 상승률/OHLC/거래량을 직접 제어, `hovermode="x unified"`. 캔들 주말 갭은 **x를 날짜 문자열 category 축**으로 두면 사라짐(rangebreaks 불필요). `to_html(include_plotlyjs="cdn")` 로 파일 ~20KB.
  4. **fig 생성과 HTML 직렬화를 분리**(`_build_review_figure`/`build_trade_review_html`)하면 kaleido `write_image` 로 정적 스냅샷을 떠 **브라우저 없이도 레이아웃을 눈으로 검증**할 수 있다.
  5. 새 의존성(plotly)은 **함수 내부 지연 import** + 호출부 try/except 로 **PNG 폴백**을 둬, 구버전 실행 환경에서도 죽지 않게.
